...
class ReviewerModelQLora(T5ForConditionalGeneration):
    ...
    def init(self):
        ...
        self.cls_head.weight.data.normal_(
    ...
    def from_pretrained(path, **kwargs):
        ...
        if not isinstance(config, T5Config):
            raise ValueError(
                f"Expected T5Config for the ReviewerModelQLora, but got {type(config)}."
            )

        # Load the base model
        base_model = T5ForConditionalGeneration.from_pretrained(path, **kwargs)

        # Prepare model for QLoRA training (int8 quantization)
        base_model = prepare_model_for_int8_training(base_model)

        # Define QLoRA configuration and apply it to the model
        ...
        # Create an instance of ReviewerModelQLora
        reviewer_model = ReviewerModelQLora(config)
        reviewer_model.model = model  # Use the base model with QLoRA as the underlying model

        # Ensure cls_head matches the configuration
        reviewer_model.cls_head = nn.Linear(
            getattr(config, "d_model", getattr(config, "hidden_size", 768)), 2, bias=True
        )
        reviewer_model.init()
    ...
    def forward(
        ...
        r"""
        Modified forward function with support for QLoRA usage
        """
        if "cls" in kwargs:
            assert (
                "input_ids" in kwargs
    ...
    def review_forward(
        ...
        if self.config.tie_word_embeddings:  # This is True by default
        ...
        if decoder_input_ids is not None:
            ...
            if encoder_loss and input_labels is not None:
                ...
                loss += cls_loss_fct(
                    cls_logits.view(-1, cls_logits.size(-1)), input_labels.view(-1)
                )
            return loss
        return cls_logits, lm_logits


class ReviewerModelPEFT(T5ForConditionalGeneration):
    ...
    def __init__(self, config):
        ...
        # Add Adapter configuration for PEFT (manual implementation as transformers.adapters is not available)
        self.adapter = nn.Sequential(
            nn.Linear(self.config.d_model, self.config.d_model // 16),
            nn.ReLU(),
            nn.Linear(self.config.d_model // 16, self.config.d_model)
        )
    ...
    def from_pretrained(path, **kwargs):
        ...
        if not isinstance(config, T5Config):
            raise ValueError(
                f"Expected T5Config for the ReviewerModelPEFT, but got {type(config)}."
            )

        model = ReviewerModelPEFT(config)
        base_model = T5ForConditionalGeneration.from_pretrained(path, **kwargs)
        model.model = base_model  # Use T5ForConditionalGeneration as the base model
    ...
    def forward(
        ...
        r"""
        Modified forward function with support for adapter usage
        """
        if "cls" in kwargs:
            assert (
                "input_ids" in kwargs
    ...
    def cls(self, input_ids, labels, attention_mask):
        ...
        first_hidden = adapted_hidden_states[:, 0, :]
        first_hidden = nn.Dropout(0.3)(first_hidden)
        logits = self.cls_head(first_hidden)
        loss_fct = CrossEntropyLoss()
        if labels is not None:
    ...
    def review_forward(
        ...
        if encoder_loss:
            cls_logits = nn.functional.linear(
                adapted_hidden_states, self.encoder.get_input_embeddings().weight
            )
        lm_logits = self.lm_head(sequence_output)
        if decoder_input_ids is not None:
            lm_loss_fct = CrossEntropyLoss(ignore_index=0)  # Warning: PAD_ID should be 0
...
def build_or_load_gen_model(args):
    ...
    config_class, model_class, tokenizer_class = T5Config, ReviewerModelQLora, RobertaTokenizer
